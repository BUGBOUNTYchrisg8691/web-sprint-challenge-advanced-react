...
test("Test that plants display in shopping cart", () => {
  render(<ShoppingCart cart={mockData} />);

  const itemName = screen.getByText(mockData[0].name);
  expect(itemName).toBeTruthy();

  const itemName1 = screen.getByText(mockData[1].name);
  expect(itemName1).toBeTruthy();

  const itemName2 = screen.getByText(mockData[2].name);
